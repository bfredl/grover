...
with tf.Session(config=tf_config, graph=tf.Graph()) as sess:
    ...
    #ignore_ids_np = np.array(encoder.special_tokens_onehot) # wrong size :(
    ignore_ids_np = np.zeros((50270,), dtype=np.float32) # why u magic number??
    ignore_ids_np[encoder.endoftext] = 1
    tokens, probs = sample(news_config=news_config, initial_context=initial_context,
                           eos_token=eos_token, ignore_ids=ignore_ids_np, p_for_topp=p_for_topp,
                           do_topk=False)
    ...
    while True:
        ...
        # Let's go!
        text = raw_text.encode("utf-8").decode("cp1252") # yeehaw!
